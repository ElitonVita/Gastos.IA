...
// Re-render every view that's built dynamically in JS (as opposed to the static
// data-i18n-tagged markup that applyTranslations() handles) so it picks up the
// current language. Shared by switchLanguage() (user clicks a flag) and initI18n()
// (first page load) so the two call lists can't drift apart.
...
async function initI18n() {
  ...
  updateLanguageSelector();
  applyTranslations();
  // The very first render pass (persistence.js's top-level load code, which runs as
  // soon as it's parsed — before this async function's await resolves) happens before
  // `translations` is populated, so every dynamic view rendered at that point shows
  // raw i18n keys / "Translation missing" warnings. Re-run the same dynamic re-render
  // list switchLanguage() uses so the app is in a correct state without needing the
  // user to click a language button first.
  reRenderDynamicContent();
}
